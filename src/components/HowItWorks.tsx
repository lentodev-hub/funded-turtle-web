...
const HowItWorks = () => (
  <section id="how" className="py-24 bg-background relative overflow-hidden">
    <div className="absolute -top-40 -right-40 w-96 h-96 bg-glow opacity-50 animate-blob" aria-hidden />
    <div className="absolute -bottom-40 -left-40 w-96 h-96 bg-glow opacity-30 animate-blob" style={{ animationDelay: "3s" }} aria-hidden />
      ...
      <div className="grid md:grid-cols-3 gap-8 relative">
        {steps.map((s, i) => (
          <Reveal key={s.n} delay={i * 150}>
            <div className="relative p-8 rounded-2xl bg-gradient-card border border-border shadow-soft hover-lift h-full">
              ...
              {i < steps.length - 1 && (
                <div className="hidden md:block absolute top-1/2 -right-4 text-3xl text-primary/40 animate-pulse">→</div>
              )}
            </div>
          </Reveal>
        ))}
